...
function charApprox(text, modelId, tokenizerId, opusMult) {
  ...
  if (tokenizerId === "gemini") ratio = base * (4.0 / 3.5);else if (tokenizerId === "o200k") ratio = base * (3.8 / 3.5);else if (tokenizerId === "llama") ratio = base * (3.6 / 3.5);
...
async function countTokensAsync(model, text, opusMult) {
  ...
  if (provider === "openai") {
    ...
    try {
      ...
      return {
        tokens: charApprox(text, model.id, model.tokenizer, opusMult),
        source: "fallback",
        error: e.message
      };
    }
